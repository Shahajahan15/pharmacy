...
$(document).on("select2:select", ".sub-customer-auto-complete", function () {
   // alert('Shamin');
   var id = $(this).val();
    //  console.log(id);
    var url = siteURL + 'main_pharmacy_sale/pharmacy/set_sub_customer/' + id;
    $.get(url, function (data) {
        //console.log(data);
        eval(data);
        // customerCheck(id, 1);
...
$(document).on("submit", ".sale_submit", function (e) {
    ...
    var sub_cust_id = $("#sub_cust_id").val();
    if (!medicine_ids) {
        return false;
    }
