...
// Filterable Permissions Matrix List
// @author A. H. Abid
$('.filter_actions').on('change', function(e){

    var matrixModuleId = $('#filter_matrix_modules').val(),
        matrixModuleItemId = $('#filter_matrix_items').val(),
        matrixModuleItemSubId = $('#filter_matrix_subs').val(),
        tableRowsObj = $('#permission_table tbody tr');


    if (matrixModuleId) {
        $('#filter_matrix_items').find('option[data-parent!="'+matrixModuleId+'"]').hide();
        $('#filter_matrix_items').find('option[value=""],option[data-parent="'+matrixModuleId+'"]').show();
    }

    if (matrixModuleItemId) {
        $('#filter_matrix_subs').find('option[data-parent!="'+matrixModuleItemId+'"]').hide();
        $('#filter_matrix_subs').find('option[value=""],option[data-parent="'+matrixModuleItemId+'"]').show();
    }

    if (matrixModuleId == "" && matrixModuleItemId == "" && matrixModuleItemSubId == "") {
        $('#filter_matrix_items').find('option').show();
        $('#filter_matrix_subs').find('option').show();
    ...
    tableRowsObj.each(function(i, row){
        var $row = $(row),
            showRow = false,
            rowModuleId = $row.attr('data-filter-mod'),
            rowItemId = $row.attr('data-filter-item'),
            rowItemSubId = $row.attr('data-filter-sub');

        if (matrixModuleId && matrixModuleItemId && matrixModuleItemSubId) {
            showRow = (rowModuleId == matrixModuleId && rowItemId == matrixModuleItemId && rowItemSubId == matrixModuleItemSubId);
        }
        else if (matrixModuleId && matrixModuleItemSubId) {
            showRow = (rowModuleId == matrixModuleId && rowItemSubId == matrixModuleItemSubId);
        }
        else if (matrixModuleId && matrixModuleItemId) {
            showRow = (rowModuleId == matrixModuleId && rowItemId == matrixModuleItemId);
        }
        else if (matrixModuleItemId && matrixModuleItemSubId) {
            showRow = (rowItemId == matrixModuleItemId && rowItemSubId == matrixModuleItemSubId);
        }
        else if (matrixModuleItemSubId) {
            showRow = (rowItemSubId == matrixModuleItemSubId);
        }
        else if (matrixModuleItemId) {
            showRow = (rowItemId == matrixModuleItemId);
        }
        else if (matrixModuleId) {
            showRow = (rowModuleId == matrixModuleId);
        }
