...
class Settings extends Admin_Controller
{
    ...
    public function permission_matrix()
    {
        ...
        $data = [];
        $data['matrix_permissions'] = $this->permission_model->select('permission_id, name')
                                                               ->order_by('name')
                                                               ->find_all();

        $data['matrix_roles'] = $this->role_model->select('role_id, role_name')
                                                 ->where('deleted', 0)
                                                 ->find_all();
        ...
        foreach ($data['matrix_permissions'] as $pKey => $permission) {
            $parts = explode('.', $permission->name, 3);

            if (!isset($data['modules'][$parts[0]])) {
                $data['modules'][$parts[0]] = [];
            }

            if (!isset($data['modules'][$parts[0]][$parts[1]])) {
                $data['modules'][$parts[0]][$parts[1]] = [];
            }

            $data['modules'][$parts[0]][$parts[1]][] = $parts[2];

            $data['matrix_permissions'][$pKey]->filter_mod = $parts[0];
            $data['matrix_permissions'][$pKey]->filter_item = $parts[1];
            $data['matrix_permissions'][$pKey]->filter_sub = $parts[2];
        }
