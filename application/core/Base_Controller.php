...
class Base_Controller extends MX_Controller
{
    ...
    public $autoload = array(
        'libraries' => array('settings/settings_lib', 'events', 'Assets'),
    ...
    protected function authenticate()
    {
        ...
        if ($this->session->userdata('role_id') == 10) {
            $where['DATE(tp.collection_date) ='] = date('Y-m-d');
            //$where['tp.counter_id ='] = $this->session->userdata('counter_id');
            $where['tp.collection_by ='] = $this->current_user->id;
            $amount = @$this->db
                 ->select('SUM(IF(tp.transaction_type = 3, 0, tp.amount)) as amount')
                 ->from('transaction_payment as tp')
                 ->where($where)
                 ->group_by('tp.collection_by')
                 ->get()->row()->amount;
            $this->session->set_userdata('collection', (int)$amount);
        }
    }
    ...
    protected function showProfiler($frontEnd = false)
    {
        ...
        if (! $isCliRequest
            ...
            if ($frontEnd == false
                ...
                $this->output->enable_profiler($frontEnd);
            }
        }
    }
}
