...
if ( ! function_exists('form_dropdown'))
{
	 ...
	 * @return string A string with the formatted input element, label tag and wrapping divs.
	 */
	function form_dropdown($data, $options=array(), $selected=array(), $label='', $extra='', $tooltip = '', $labelExtra='', $isAjaxDropDown=0)
	{
        ...
        $labelExtra = (empty($labelExtra))? 'class="control-label"' : $labelExtra;

		
		$output = <<<EOL
<div class="control-group {$error}">
	<label {$labelExtra} for="{$data['id']}">{$label}</label>
	<div class="controls">
		 <select {$output} {$extra}>
			{$options_vals}
		</select>
