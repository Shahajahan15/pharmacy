...
class Settings extends Admin_Controller
{
    ...
    public function __construct()
    {
		...
		//$this->load->model('pathology/lab_model');
    ...
    public function create()
    {
        ...
        $employee_id =  empty($_REQUEST['employee_id']) ? 0 : (int)$_REQUEST['employee_id'];
        if ($employee_id) {
			 $employee_info = $this->employee_model->find($employee_id);
			 $department = $this->getDepartmentInfo($employee_id);
			 $employee = $this->getEmployeeInfo($employee_id);
			 Template::set('employee_info', $employee_info);
			 Template::set('department', $department);
		} else {
			$employee = $this->getEmployeeInfo();
			Template::set('department', $this->department_model->find_all());
		}
		...
		Template::set('branch_name', $this->branch_info_model->find_all_by('IS_DELETED',0));
		//Template::set('lab_room', $this->lab_model->find_all());
		//Template::set('store_list', $this->store_model->find_all());
    ...
    private function getEmployeeInfo($employee_id = 0)
    {
    	if ($employee_id){
		...
		$employee = $this->db
        				->select('EMP_ID as id,	EMP_NAME as emp_name')
        				->where($where)
        				->get('hrm_ls_employee')
        				->result();
        return $employee;
    ...
    private function getDepartmentInfo($employee_id)
    {
		$result = $this->db
					->select('d.dept_id, d.department_name')
					->from('hrm_ls_employee as emp')
					->where('emp.EMP_ID',$employee_id)
					->join('lib_department as d','emp.EMP_DEPARTMENT = d.dept_id')
					->get()
					->result();
		return $result;
	}
    
    public function getEmployeeNameByDepartmentId()
    {
		...
		$result = $this->db
					...
					->result();
		$options = '';
		$options .= '<select name="employee_id" id="employee_id" required="">';
	...
	public function getDifferentIdByTypeId()
	{
		$type = $this->input->post('type',TRUE);
		/* sample */
		$name = "";
		if (($type == 1) or ($type == 2)) {
			$name = ($type == 1)? "Sample Room" : "Lab Room";
			$result = $this->db
		...
		$options = '';
		$options .= '<select name="type_id" class="form-control" id="type_id" required="">';
		$options .= '<option value="0">Select '.$name.'</option>';
		if ($result) {
			foreach ($result as $row) {
    ...
    public function edit($userId = '')
    {
        ...
        $employee = $this->db
        				->select('EMP_ID as id,	EMP_NAME as emp_name')
        				->where('EMP_DEPARTMENT',$user->department_id)
        				->get('hrm_ls_employee')
        				->result();
        /*type */
        if ($user->type) {
			...
			$name = ($type == 1)? "Sample Room" : "Lab Room";
			if ($type ==1) {
				$select_type = $user->sample_room_id;
			} else {
				$select_type = $user->lab_room_id;
			}
			$result = $this->db
					  ->select('id,	lab_name as name')
					  ->get('pathology_lab')
			 ...
			 Template::set('type_name', $result);
			 Template::set('select_name', $name);
			 Template::set('select_type', $select_type);
		}
		...
		//Template::set('store_list', $this->store_model->find_all());
        Template::set('roles', $this->role_model->select('role_id, role_name, default')->where('deleted', 0)->find_all());
        Template::set('employee_id', empty($_REQUEST['employee_id']) ? 0 : (int)$_REQUEST['employee_id']);
    ...
    private function saveUser($type = 'insert', $id = 0, $metaFields = array(), $currentRoleName = '')
    {
       ...
       // echo $data['type'];
        if ($data['type'] == 1) {
			$data['sample_room_id'] = $data['type_id'];
		} elseif ($data['type'] == 2) {
			$data['lab_room_id'] = $data['type_id'];
		} elseif ($data['type'] == 3) {
			$data['store_id'] = $data['type_id'];
		}elseif ($data['type'] == 4) {
			$data['pharmacy_id'] = $data['type_id'];
		}
		//unset($data['type']);
		unset($data['type_id']);
       // echo '<pre>';print_r($data);exit;
        ...
        if ($type == 'insert') {
        	...
        	$type_info = $this->user_model->find($id);
        	/*if ($type_info && $type_info->type) {
				 if ($type_info->type == 1) {
					$data['sample_room_id'] = 0;
				} elseif ($type_info->type == 2) {
					$data['lab_room_id'] = 0;
				} elseif ($type_info->type == 3) {
					$data['store_id'] = 0;
				}elseif ($type_info->type == 4) {
					$data['pharmacy_id'] = 0;
				}
			} */
