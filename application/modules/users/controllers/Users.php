...
class Users extends Front_Controller
{
    ...
    public function login()
    {
        ...
        if (isset($_POST['log-me-in'])
            ...
            //only for counter role type user redirect here
            if ($this->session->userdata('role_id') == 10 && !$this->session->has_userdata('counter_id')) {
                Template::redirect('/admin/counter/users');
            }
    ...
    public function logout()
    {
        ...
        if ($this->session->userdata('role_id') == 10 && $this->session->has_userdata('counter_id')
            && ENVIRONMENT != 'development' && $this->cache->{$adapter}->is_supported()) {
            //$this->db->where(array('counter_id' => $this->session->userdata('counter_id')))->update('bf_lib_counter', array('is_booked' => 0));
            $key = 'counter:' . $this->session->userdata('counter_id');
