...
class Auth
{
    ...
    {
        if ($this->user AND $this->user->username == 'admin') {
            return true;
        }
    ...
    private function setupSession(
        ...
        $this->ci->load->model('hrm/employee_model');
        $emp = $this->ci->employee_model->find($user->employee_id);
        ...
        $this->ci->session->set_userdata(
            array(
                ...
                'role_id'     => $role_id,
                'logged_in'   => true,
                'image_profile'=> !empty($emp) ? $emp->EMP_PHOTO : '',
                'email'       => $email,
    ...
    private function autologin()
    {
        ...
        $this->ci->load->model('users/user_model');
        $user = $this->ci->user_model->select(array('id', 'username', 'email', 'password_hash', 'display_name', 'users.role_id', 'users.role_name'))
                                     ->find($user_id);
