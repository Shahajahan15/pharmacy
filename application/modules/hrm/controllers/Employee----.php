...
// 			$search_box['employee_name'] 		= $this->input->post('employee_name');
...
// 		$getEmployeeListObj 		= new GetEmployeeListService($this);		
// 		$records 					= $getEmployeeListObj	
									
// 									->setEmpMobile($search_box['employee_name'])
// 									->setLimit($search_box['limit'])
// 									->setOffset($offset)
// 									->execute();				
...
// 		$this->pager['per_page'] 			= $search_box['limit'];
...
// 		$data['empType'] 	=$this->config->item('emp_type');	
// 		$data['records'] = $records;
// 		if ($this->input->is_ajax_request()) {
//             echo $this->load->view('employee/employee_list', $data);
//             return;
//         }
...
//     }
	...
	public function show_list()
    {
        ...
        if($this->input->post('employee_name')){
            $con['hrm_ls_employee.EMP_NAME like']='%'.trim($this->input->post('employee_name')).'%';
        }
         if($this->input->post('common_text_search')){
            $con['hrm_ls_employee.EMP_CODE like']='%'.trim($this->input->post('common_text_search')).'%';
        }
         if($this->input->post('empType_list')){
            $con['hrm_ls_employee.EMP_TYPE']=$this->input->post('empType_list');
        }
         if($this->input->post('designation_list')){
            $con['hrm_ls_employee.EMP_DESIGNATION']=$this->input->post('designation_list');
        }
         if($this->input->post('test_department_list')){
            $con['hrm_ls_employee.EMP_DEPARTMENT']=$this->input->post('test_department_list');
        }
	...
	public function view_employee(){
       ...
       $this->db->select("
			...
			->get()
			->result();			
			//$edu_info = $this->employee_model->find($id);
  ...
  //      $data=array();
	...
	{	
		...
		if(intval($areaId) > 0 && intval($areaId) > 0)
		{
			...
			*/
			...
			{				
			$options = array();
			foreach($area_list as $result){ $options[$result["trt_id"]] = $result["trt_name"]; }			
    ...
    {				
		...
		{		
			...
			$data['relation'] 				= $this->config->item('relation');
	...
	{		
		...
		if($relationtype){
			...
			} else
			{
				...
				if($result == 0 )
				{				
					$this->employee_family_model->insert($data);					
				}															
			}			
		}							
	...
	{		
		if($EMP_FAMILY_ID        = (int)$this->input->post('EMP_FAMILY_ID_EDIT'))
		{
		    ...
		    echo "
				$('#NAME').val('$NAME');
				$('#NAME_BENGALI').val('$NAME_BENGALI');
				$('#RELATION').val('$RELATION');
				$('#BIRTH_DATE').val('$BIRTH_DATE');
				$('#AGE').val('$AGE');
				$('#OCCPATION').val('$OCCPATION');
				$('#OCCPATION_BENGALI').val('$OCCPATION_BENGALI');
...
//=================== Job Experience info start ========================
    ...
    {				
        ...
        if($employeeId)
		{		
			$records = $this->emp_job_experience_model->find_by('hrm_ls_emp_job_experience.EMP_ID',$employeeId);	$this->load->model('library/designation_info_model', NULL, TRUE);			
			$data = array();				
	...
	{		
		...
		foreach($records as $recorded): $jobExperienceRecord = (object) $recorded;																											$EMP_JOB_EXP_ID=$jobExperienceRecord->EMP_JOB_EXP_ID;
		$empId 			= $this->input->post('employeeId');
			echo "			
				<tr>
