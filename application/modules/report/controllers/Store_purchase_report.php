...
class Store_purchase_report extends Admin_Controller 
{
  ...
  public function index()
    { 
     ...
     if($this->input->post('supplier_name')){
      ...
      $sql = "
            ...
            FROM 
               ...
               SELECT
                 ...
                 (SELECT 
                  ...
                  FROM
                    bf_store_purchase_order_received_dtls as rd where rd.master_id=rms.id
               ...
               FROM
                  bf_store_purchase_order_received_mst as rms
                    
               ) as rm   

            JOIN  
                bf_store_supplier_mst as s ON s.SUPPLIER_ID=rm.supplier_id
            JOIN  
                bf_store_supplyer_dtls as sd ON sd.STORE_SUPPLIER_DTLS_ID=s.SUPPLIER_ID
            JOIN
                bf_store_product_company AS c on c.COMPANY_ID = sd.STORE_SUPPLIER_DTLS_ID    
          /*for paid amount*/
          LEFT JOIN 
                 (
                 SELECT
                   ...
                   (SELECT 
                      sum(pd.paid) 
                    FROM
                       bf_store_payment_dtls as pd where pd.master_id = pms.id
                    GROUP BY pd.master_id
                         
                  ) as  paid
                 FROM  
                   bf_store_payment_mst as pms
                  )
        ...
        $this->pager['base_url'] = site_url() . '/admin/store_purchase_report/report/index' . '?';
        $this->pager['total_rows'] = $total;
        $this->pager['per_page'] = $limit;
        $this->pager['page_query_string'] = TRUE;
        $this->pagination->initialize($this->pager);
        $list_view = 'store_purchase_report/index';
       ...
       Template::set($data);
        Template::set_block('sub_nav', 'store_purchase_report/_sub_report');
        Template::set_view('report_template');
        Template::render();
    }	

    public function per_payment_details($supplier_id)
    {
    ...
    $sql = "
      ...
      $total = $this->db->query("SELECT FOUND_ROWS() as count")->row()->count;
      $this->pager['base_url'] = site_url()."/admin/store_purchase_report/report/per_payment_details/$supplier_id".'?';
      $this->pager['total_rows'] = $total;
      $this->pager['per_page'] = $limit;
      $this->pager['page_query_string'] = TRUE;
      $this->pagination->initialize($this->pager);
      ...
      $list_view = 'store_purchase_report/per_payment_details';
      $data['search_box'] = $search_box;
      $data['records'] = $records;
      ...
      if ($this->input->is_ajax_request()) {
        if(isset($_POST['print'])){
        echo $this->load->view('store_purchase_report/per_payment_details', $data, true);
                             exit;
          }
         echo $this->load->view($list_view, $data, true);
                                   exit;
  ...
  public function per_pay_details_details($master_id)
    ...
    $sql = "
            SELECT 
                   ...
                   0 as paid
            FROM
                   bf_store_purchase_order_received_mst as rm
            JOIN
                   bf_store_purchase_order_received_dtls as rd ON rd.master_id=rm.id
            JOIN  
                   bf_store_supplier_mst as s ON s.supplier_id=rm.supplier_id        
            JOIN
                   bf_store_products AS p ON p.id = rd.product_id
            where 
                   rm.id = $master_id                       
     ...
     UNION
             ...
             FROM
                   bf_store_payment_mst as pm
             JOIN
                   bf_store_payment_dtls as pd on pd.master_id=pm.id
             JOIN  
                   bf_store_supplier_mst as s ON s.supplier_id=pm.supplier_id               
      ...
      $records = $this->db->query($sql)->result();  
       //echo "<pre>"; print_r($records); exit();
      $list_view = 'store_purchase_report/store_com_details_details';
