...
class Store_stock_dept extends Admin_Controller
{
    ...
    public function stocks($dept_id = 0, $offset = 0)
    {
      ...
      /*  if (trim($this->input->post('store_product_id'))) {
        ...
        $data['total_stocks'] = $this->getTotalDepartmentStock(0);
        $data['records'] = $this->department_stock_model->getDeptStocks();

		$this->load->library('pagination');
        $total = $this->db->query("SELECT FOUND_ROWS() as count")->row()->count;
		//$this->pager['base_url'] = site_url('admin/store_stock_dept/report/stocks/'.$dept_id);
        $this->pager['base_url'] = site_url('admin/store/report/store_stock_dept/stocks');
		...
		if ($this->input->is_ajax_request()) {
            $data['dept_id'] = $this->input->post('test_department_list', true);
            $data['total_stocks'] = $this->getTotalDepartmentStock($data['dept_id']);
            $data['department'] = $this->db
                                    ->where('dept_id', $data['dept_id'])
                                    ->get('bf_lib_department')
                                    ->row();
        ...
        Template::set($data);
		Template::set('toolbar_title', 'Department Stock Report');
        Template::set_view('report_template');
        Template::set_block('sub_nav', 'store/stock/dept/_sub_report_dept');
        Template::render();
    }

    private function getTotalDepartmentStock($dept_id = 0)
    {
        $con = [];
        if ($dept_id) {
            $con ['ds.dept_id'] = $dept_id;
        }
        $records = $this->db
                        ->select('IFNULL(ROUND(SUM(ds.quantity_level)), 0) AS total_stock, IFNULL(ROUND(SUM(ds.quantity_level * p.purchase_price)), 0) as total_stock_tk')
                        ->from('store_department_stock as ds')
                        ->join('store_products as p','ds.product_id = p.id')
                        ->where($con)
                        ->get()
                        ->row();
        return $records;
    ...
    public function details($dept_id, $product_id)
    {
        ...
        if (!$product_id || !$dept_id) {
            redirect('admin/store/report/store_stock_dept/stocks');
        }
		...
		$search_box['common_text_search_flag'] = 1;
        $search_box['common_text_search_label'] = 'Employee Name';
        ...
        $data['records'] = $this->product_model->getDepartmentStockDetails($filter, $limit, $offset);
        $data['search_box'] = $search_box;
		$data['list_view'] = 'store/stock/dept/dept_details_stocks_report';
        $total = $this->db->query("SELECT FOUND_ROWS() as count")->row()->count;
		$this->load->library('pagination');
		$this->pager['base_url'] = site_url('admin/store/report/store_stock_dept/details/'.$dept_id.'/'.$product_id);
		$this->pager['total_rows'] = $total;
		$this->pager['per_page'] = $limit;
		$this->pager['page_query_string'] = TRUE;
		$this->pagination->initialize($this->pager);
        ...
        Template::set($data);
		Template::set('toolbar_title', 'Department Stock Details Report');
        Template::set_view('report_template');
