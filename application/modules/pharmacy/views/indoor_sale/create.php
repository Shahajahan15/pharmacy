    ...
    <?php echo form_open($this->uri->uri_string(), 'role="form", class="nform-horizontal form_reset sale_submit", data-toggle="validator"'); ?>
                        	...
                        	<table class="table table-bordered" style="margin-bottom:0px; ">
								...
								<tbody>
									<tr class="info">
										...
										<td><?php echo $patient_info->admission_code; ?>
											<input type="hidden" value="<?php echo $patient_info->admission_id; ?>" id="admission_id" name="admission_id" />
										</td>
										...
										<td><?php echo ($patient_info->sex == 1) ? "Male" : "Female"; ?></td>
			             ...
			             <tbody id="sale_data"> 
			             	...
			             	foreach ($medicine_info as $key => $row) :
			             	 // if ($row->stock > 0) : 
			             		...
			             		$qnty = ($row->stock < $row->qnty)? (int) $row->stock : (int) $row->qnty;
                               ...
                               if($over_all_discount)
                                  ...
                                  $nd_discount_type  = 1;
                               	...
                               	{
                               		$service_discount =  patient_discount($patient_id,2,$row->medicine_id);
			             		...
			             		// $sd_percent = patient_discount($patient_id,2,$row->medicine_id)->discount;
			             		//print_r($sd_percent);
			             	...
			             	?>
				             <tr class="success">
				             	<td><?php echo $row->product_name.'&nbsp; >> &nbsp;'.$row->category_name; ?><input type="hidden" name="product_id[]" class="product_id" value="<?php echo $row->medicine_id; ?>"> </td>
				             	...
				             	?><input type="hidden" name="stock[]" class="stock" value="<?php echo isset($row->stock) ? $row->stock : 0; ?>"> </td>
				             	<td><?php echo $nd_percent; ?><input type="hidden" name="nd_percent[]" value="<?php echo $nd_percent; ?>"> <input type="hidden" name="nd_discount_id[]" value="<?php echo $nd_discount_id; ?>"><input type="hidden" name="nd_discount_type[]" value="<?php echo $nd_discount_type; ?>"></td>
				             	<td><?php echo $nd_amount; ?><input type="hidden" name="nd_amount[]" class="nd_amount" value="<?php echo $nd_amount; ?>"> </td>
					...
					<fieldset>
						...
						<div class="<?php echo form_error('pharmacy_total_price') ? 'error' : ''; ?>">
							<div class='form-group'>
							   ...
							   <input class="form-control" id="pharmacy_total_price" name="pharmacy_total_price" value="<?php echo round($total,0); ?>" type="text" readonly="" />
							</div>
						</div>		
						</div>

						<div class="col-xs-4 col-sm-3 col-md-2 col-lg-2">
						<div class="<?php echo form_error('cost_paid_by') ? 'error' : ''; ?>">
							<div class='form-group'>
							   <label>Cost Paid By<span class="required">*</span></label>
							   <input class="form-control" id="cost_paid_by" name="cost_paid_by" value="<?php echo $patient_info->cost_paid_by; ?>" type="hidden" readonly="" />

							   <input class="form-control" value="<?php echo ($patient_info->cost_paid_by==1)? 'Patient':'Hospital'; ?>" type="text" readonly="" />
							</div>
						</div>		
						</div>
						...
						<div class="<?php echo form_error('pharmacy_total_paid') ? 'error' : ''; ?>">
							<div class='form-group'>
							   ...
							   <input class="form-control decimalmask"  autocomplete="off" id="pharmacy_total_paid" name="pharmacy_total_paid" type="text" value="0" />
							</div>
						</div>		
						</div>
						...
						<div class="<?php echo form_error('pharmacy_total_less_discount') ? 'error' : ''; ?>">
							<div class='form-group'>
							   ...
							   <input class="form-control decimalmask" autocomplete="off" id="pharmacy_total_less_discount" name="pharmacy_total_less_discount"  value="0" type="text" />
							</div>
						</div>		
						</div>
						<div class="col-xs-4 col-sm-3 col-md-3 col-lg-3">
						<div class="<?php echo form_error('pharmacy_total_due') ? 'error' : ''; ?>">
							<div class='form-group'>
							   ...
							   <input class="form-control" id="pharmacy_total_due" name="pharmacy_total_due" type="text" value="<?php echo round($total,0); ?>" readonly="" />
							</div>
						</div>		
						</div>
					</fieldset>
            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    ...
                    <?php echo lang('bf_or'); ?>
                    <input type="Reset" onClick="resetMPForm()" class="btn btn-warning btn-sm" value="Reset">                  
                    </div>   
                </div>
                </fieldset>
            </div>
