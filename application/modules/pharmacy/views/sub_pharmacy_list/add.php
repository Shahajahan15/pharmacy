...
<input  type="hidden" value="<?php echo ($customer_type == 1) ? $record->client_id : 0; ?>" name="admission_id" class="admission_id"/>
<input  type="hidden" value="<?php echo ($customer_type == 3) ? $record->client_id : 0;; ?>" name="customer_id" class="customer_id"/>
<input  type="hidden" value="<?php echo ($customer_type == 2) ? $record->client_id : 0;; ?>" name="patient_id" class="patient_id"/>
<input  type="hidden" value="<?php echo ($customer_type == 4 || $customer_type == 5) ? $record->client_id : 0;; ?>" name="employee_id" class="employee_id"/>
...
<table class="table table-bordered" style="margin-bottom:0px; ">
		<tbody>
			...
			<tr style="text-align: center;">
				<td colspan="5"><input type="submit" name="mr_submit" value="Done" class="btn btn-primary btn-xs" id="m_report_add">
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('form.mr_form_submit').validator().on('submit', function(e){
          if (!e.isDefaultPrevented()) {
                ...
                var targetUrl = siteURL +"sub_pharmacy_sale_list/pharmacy/add";
                $.ajax({
			        url: targetUrl,
			        type: "POST",
			        data: sendData,
       ...
       ajax: {
           ...
           url: siteURL + "sub_pharmacy_sale_list/pharmacy/getMedicineDueSaleNoByKey"+"/"+$(".customer_type").val()+"/"+$(".client_id").val(),
            dataType: 'json',
            delay: 250,
	...
	/*$(document).on('keyup', '.overall_discount', function(){
		var overall_discount = parseFloat($(this).val());
	});*/
