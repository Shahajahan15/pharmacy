...
$(document).on('click','.sale_no',function(e){
		...
		var targetUrl = siteURL +"sub_pharmacy_sale_return/pharmacy/getSaleInfo";
		$.ajax({
        url: targetUrl,
        type: "POST",
        data: sendData,
        ...
        success: function (response) {
			...
			$("#total_less_dsc").val(response.tot_less_discount);
			...
			//$("#return_due_info").val(response.return_due_info.tot_return_due);
			$("#return_less_info").val(response.return_less_info);
			}
...
$(document).on("submit",".sale_return_submit",function(e){
   		...
   		var targetUrl = siteURL +"sub_pharmacy_sale_return/pharmacy/save";
   		$.ajax({
        url: targetUrl,
        type: "POST",
        data: sendData,
...
$(document).on('keyup','.r_qnty',function(){
	...
	var return_less_infor=parseFloat($('#return_less_info').val());
	...
	var r_discount_amount = parseFloat(per_qnty_dsc * r_qnty);
	if (qnty < r_qnty) {
	 	$(this).val(qnty);
	 	var r_discount_amount = parseFloat(per_qnty_dsc * qnty);
	 	var qnty = r_qnty;
	 }

console.log("qnty="+r_qnty);
	...
	$(this).closest('tr').find('.r_discount_amount').val(r_discount_amount);
...
$(document).on('click','.s_remove',function(){
	...
	function total(){
		...
		tot_dsc_amount = 0;
		$('form table tbody .r_discount_amount').each(function(){
		...
		//var return_paid =(p_due) - (tot_r_amount - tot_dsc_amount);
