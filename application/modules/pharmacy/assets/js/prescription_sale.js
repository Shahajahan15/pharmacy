$('#pharmacy_category').change(function(){
	...
	//targetUrl=siteURL+'prescription_sale/pharmacy/getSubCategoryByCategoryId/'+cat_id;
	targetUrl=siteURL+'prescription_sale/pharmacy/getProductByCategoryId/'+cat_id;
...
$(document).on('change','#pharmacy_sub_category',function(){
	targetUrl=siteURL+'prescription_sale/pharmacy/getProductBySubCategoryId/'+$(this).val();
	$.get(targetUrl,function(data){
		$('#pharmacy_medicine').html(data);
	})
});
...
$(document).on('click','.p_print',function(){
	...
	//alert(id);return;
	targetUrl=siteURL+'prescription_sale/pharmacy/prescription_print/'+id;
...
$('#pharmacy_customer_type').change(function(){
	...
	targetUrl=siteURL+'prescription_sale/pharmacy/getCustomerType';
...
$(document).on("submit",".sale_submit",function(e){
   		...
   		var targetUrl = siteURL +"prescription_sale/pharmacy/save";
   		$.ajax({
        url: targetUrl,
        type: "POST",
        data: sendData,
        ...
        success: function (response) {
        	...
        	if (response.success == true) {
        		//$(".sale_submit")[0].reset();
        		//$("#sale_data").empty();
        		window.location = siteURL +"prescription_sale/pharmacy/show_list";
        		//print_view(response.print);
                showMessages(response.message, 'success');
            } else {
                showMessages(response.message, 'error');
...
// $(document).on('change','#pharmacy_medicine',function(e){
...
$(document).on('select2:select','.medicine-auto-complete',function(e){
	...
	targetUrl=siteURL+'prescription_sale/pharmacy/getMedicineInfo/'+product_id+"/" +patient_id;
	$.get(targetUrl,function(response){
	...
	row+='<td>'+response.n_discount_amount+'<input type="hidden" class="nd_amount" name="nd_amount[]" value="'+response.n_discount_amount+'" /> <input type="hidden" class="nd_discount_id" name="nd_discount_id[]" value="'+response.n_discount_id+'" /><input type="hidden" class="nd_discount_type" name="nd_discount_type[]" value="'+response.n_discount_type+'" /></td>';
	row+='<td>'+response.s_discount_percent+'<input type="hidden" name="sd_percent[]" value="'+response.s_discount_percent+'" /> <input type="hidden" class="sd_discount_id" name="sd_discount_id[]" value="'+response.s_discount_id+'" /> <input type="hidden" class="sd_discount_type" name="sd_discount_type[]" value="'+response.s_discount_type+'" /></td>';
