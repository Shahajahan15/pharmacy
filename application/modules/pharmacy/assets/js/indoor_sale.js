$('#pharmacy_category').change(function(){
	...
	//targetUrl=siteURL+'indoor_sale/pharmacy/getSubCategoryByCategoryId/'+cat_id;
	targetUrl=siteURL+'indoor_sale/pharmacy/getProductByCategoryId/'+cat_id;
...
$(document).on('change','#pharmacy_sub_category',function(){
	targetUrl=siteURL+'indoor_sale/pharmacy/getProductBySubCategoryId/'+$(this).val();
	$.get(targetUrl,function(data){
		$('#pharmacy_medicine').html(data);
	})
});
...
$(document).on("submit",".sale_submit",function(e){
   		...
   		var targetUrl = siteURL +"indoor_sale/pharmacy/save";
   		$.ajax({
        url: targetUrl,
        type: "POST",
        data: sendData,
        ...
        success: function (response) {
        	...
        	if (response.success == true) {
        		window.location = siteURL +"indoor_sale/pharmacy/show_list";
        		/*$(".sale_submit")[0].reset();
        		$("#sale_data").empty();
...
$(document).on('select2:select','.medicine-auto-complete',function(e){
	...
	targetUrl=siteURL+'indoor_sale/pharmacy/getMedicineInfo/'+product_id+"/" +patient_id;
	...
	row+='<td>0</td>';
	row+='<td><input type="text" name="qnty[]" class="form-control s_qnty decimalmask" autocomplete="off" value="'+response.qnty+'" style="width:50px;" required=""/></td>';
