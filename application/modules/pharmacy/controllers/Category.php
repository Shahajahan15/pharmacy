...
class Category extends Admin_Controller
{
    ...
    {
        $this->auth->restrict('Pharmacy.Category.Create');
        ...
        $data['form_action_url'] = site_url('admin/category/pharmacy/create');
        $data['list_action_url'] = site_url('admin/category/pharmacy/show_list');
        Template::set($data);
    ...
    public function show_list()
    {
        ...
        if (is_array($checked) && count($checked)) {
            $this->auth->restrict('Pharmacy.Category.Delete');
        ...
        $records = $this->db->select("
                           ...
                           pc.id      
                          ", false)
            ->from('bf_pharmacy_category as pc')
            ->limit($limit, $offset)
            ->order_by('pc.id', 'desc')
            ->get()
            ->result();
        ...
        $this->pager['base_url'] = site_url() . '/admin/category/pharmacy/form_list' . '?';
        ...
        $form_view = 'category/category_list';
    ...
    public function create()
    {
        ...
        if (isset($_POST['save'])) {
            if ($insert_id = $this->save_details()) {
                ...
                Template::set_message(lang('bf_msg_create_failure') . $this->category_model->error, 'error');
            }
        }
        $form_view = 'category/category_create';
        if ($this->input->is_ajax_request()) {
            echo $this->load->view($form_view, $data, true);
            exit;
        }
    ...
    private function save_details($type = 'insert', $id = 0)
    {
        ...
        $data = array();
        $data['category_name'] = $this->input->post('pharmacy_category_name');
        ...
        if ($type == 'insert') {
            ...
            $this->auth->restrict('Pharmacy.Category.Edit');
    ...
    public function edit()
    {
        ...
        $data['category_details'] = $this->category_model->find($id);
        $form_view = 'category/category_create';
