		...
		<table class="table table-striped">
			<thead>
				<tr>
					...
					<?php endif;?>					
					<th width="85%"><?php echo lang('library_leave_name'); ?></th>
					<th width="15%"><?php echo lang('library_leave_status'); ?></th>
				</tr>
			</thead>
			<?php if ($has_records) : ?>
			<tfoot>
			...
			<tbody>
				...
				<tr>
					<?php if ($can_delete) : ?>
					<td class="column-check"><input type="checkbox" name="checked[]" value="<?php echo $record->LEAVE_ID; ?>" /></td>
					<?php endif;?>
					
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/leave_info/library/leave_edit/' . $record->LEAVE_ID, '<span class="glyphicon glyphicon-pencil"></span>' .  $record->LEAVE_TYPE); ?></td>
				<?php else : ?>
