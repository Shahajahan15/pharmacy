		...
		<table class="table table-striped">
			<thead>
				<tr>
					...
					<?php endif;?>
					<th><?php echo lang('library_zone_trt_name'); ?></th>
			...
			<tbody>
				...
				<tr>
					<?php if ($can_delete) : ?>
					<td class="column-check"><input type="checkbox" name="checked[]" value="<?php echo $record->trt_id; ?>" /></td>
					<?php endif;?>
					
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/trtarea/library/edit/' . $record->trt_id, '<span class="glyphicon glyphicon-pencil"></span>'.$record->trt_name); ?></td>
				<?php else : ?>
					<td><?php e($record->trt_name); ?></td>
				<?php endif; ?>
					  
					  <td><?php e($record->area_name) ?></td>
					  <td><?php e($record->district_name); ?></td>
					  <td><?php e($record->division_name) ?></td>
					  <td><?php if($record->trt_status==1){ e("Active");}else{e("In Active");} ?></td>
				</tr>
				<?php
					endforeach;
				else:
