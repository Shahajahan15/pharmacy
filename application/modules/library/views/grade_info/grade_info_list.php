...
$GRADE_ID = isset($grade_details['GRADE_ID']) ? $grade_details['GRADE_ID'] : '';

?>

<?php
$num_columns	= 8;
$can_delete		= $this->auth->has_permission('Library.Grade.Delete');
$can_edit		= $this->auth->has_permission('Library.Grade.Edit');
		...
		<table class="table table-striped">
			<thead>
				<tr>
					...
					<th width="35%"><?php echo lang('library_grade_name'); ?></th>
					<th width="50%"><?php echo lang('library_grade_description'); ?></th>
			...
			<tbody>
				...
				<tr>
					<?php if ($can_delete) : ?>
					<td class="column-check"><input type="checkbox" name="checked[]" value="<?php echo $record->GRADE_ID; ?>" /></td>
					<?php endif;?>
					
					<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/grade_info/library/grade_edit/' . $record->GRADE_ID, '<span class="glyphicon glyphicon-pencil"></span>' .  $record->GRADE_NAME); ?></td>
					<?php else : ?>
