...
class Medicine_category extends Admin_Controller
{
	/**
	 * Constructor
	 * @return void
	...
	public function __construct()
	{
		...
		$this->load->model('medicine_category_model', null, true);	
		
		
		Template::set_block('sub_nav', 'medicine_category/_sub_nav_medicine_category');
	}
    ...
    public function show_list()
    {
    	...
    	$data['records']=$this->db->where('status',1)->where('is_deleted',0)->get('bf_medicine_categories')->result();

    	Template::set('toolbar_title','Category List');		
        Template::set_view('medicine_category/show_list');
        Template::set($data);
        Template::render();
    }
    ...
    public function create()
    {
        if (isset($_POST['save']))
        {
			if ($insert_id = $this->save())
			{
				...
				redirect(SITE_AREA .'/medicine_category/library/show_list');
			}
			else
			{
		...
		Template::set('toolbar_title','Add new Category Here');
        Template::set_view('medicine_category/create');
        Template::render();
    }
	...
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/medicine_category/library/show_companylist');
		}
	...
	private function save($type='insert', $id=0)
	{
		...
		$data = array();
		$data['category_name']        	= $this->input->post('medicine_category_name');	
		...
		if ($type == 'insert')
		{
			...
			$id = $this->medicine_category_model->insert($data);
			if (is_numeric($id)){
				$return = $id;
			}else{
				$return = FALSE;
		...
		elseif ($type == 'update'){
			...
			$return = $this->medicine_category_model->update($id, $data);
		}
